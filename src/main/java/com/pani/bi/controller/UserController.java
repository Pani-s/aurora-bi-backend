...
import com.pani.bi.manager.CosManager;
import com.pani.bi.model.dto.user.*;
...
//        LoginUserVO loginUserVO = userService.userLogin(userAccount, userPassword, request);
     ...
     * @return
     */
    @PostMapping("/update/my_with_avatar")
    public BaseResponse<Boolean> updateMyUserAvatar(@RequestPart(value = "file", required = false) MultipartFile file,
        ...
        if (file != null && file.getSize() != 0) {
            ThrowUtils.throwIf(file.getSize() > FileConstant.ONE_MB, ErrorCode.PARAMS_ERROR, "文件过大，超过 1M");
            String suffix = FileUtil.getSuffix(file.getOriginalFilename());
            ThrowUtils.throwIf(!validFileSuffixList.contains(suffix), ErrorCode.PARAMS_ERROR, "文件后缀不合法");
            try {
                String avatarUrl = cosManager.putObject(file.getInputStream(), file.getOriginalFilename());
